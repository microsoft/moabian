...
def meters_to_pixels(vec, frame_size=256, field_of_view=1.0):
    # The plate is default roughly 105% of the field of view
    plate_diameter_meters = 0.225
    plate_diameter_pixels = frame_size * field_of_view
    conversion = plate_diameter_meters / plate_diameter_pixels
        ...
        if len(contours) > 0:
            ...
            if ball_min < norm_radius < ball_max:
                ...
                if debug:
                    ...
                    if axis is not None:
                        # Draw the axis (xy plate offsets)
                        x, y = meters_to_pixels(axis)
                        x, y = list(Vector2(x, y).rotate(30))
                        draw_crosshairs(img, (int(x), int(y)))
